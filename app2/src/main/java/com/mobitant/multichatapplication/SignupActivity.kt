    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        signupActivity_button_signup.onClick {
            if(signupActivity_edittext_email.text.toString()=="" ||
                ...
                signupActivity_imageview_profile == null
                    )
            { toast("데이터를 입력하세요")
                return@onClick }
            ...
            FirebaseAuth.getInstance()
                ...
                .addOnCompleteListener(this@SignupActivity){task->
                    ...
                    //firebaseStorage에 이미지가 성공적으로 저장되면 firebaseDatabase에 userModel객체를 저장한다.
                    FirebaseStorage.getInstance().reference.child("userImage").child(uid).putFile(imageUri!!)
                        .addOnCompleteListener{task ->
                            Log.d ("TAG", "2222222222222222222222222222222222222222222222$imageUri" )
                            val imageUri = task.result!!.storage.downloadUrl
                                val userModel = UserModel()
                            Log.d ("TAG", "3333333333333333333333333333333333333333333333333$imageUri" )
                            userModel.userName = signupActivity_edittext_name!!.text.toString()
                            userModel.profileImageUrl = imageUri.toString()
                            userModel.uid = FirebaseAuth.getInstance().currentUser!!.uid

                            Log.d ("TAG", "4444444444444444444444444444444444444444444444444444444$imageUri" )
                                FirebaseDatabase.getInstance().reference.child("users").child(uid).setValue(userModel).addOnSuccessListener { finish() }
